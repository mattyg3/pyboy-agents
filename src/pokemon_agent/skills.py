...
class SkillExecutor:
    ...
    def execute(self, plan): #, state
        ...
        if t == "PRESS_A":
            self._press("a", hold_frames=5)
            return "pressed_a"
        
        if t == "PRESS_B":
            self._press("b", hold_frames=5)
            return "pressed_b"

        if t == "GO_UP":
            self._press("up", hold_frames=5)
            return "moved_up"
        
        if t == "GO_DOWN":
            self._press("down", hold_frames=5)
            return "moved_down"
        
        if t == "GO_LEFT":
            self._press("left", hold_frames=5)
            return "moved_left"

        if t == "GO_RIGHT":
            self._press("right", hold_frames=5)
            return "moved_right"

        if t == "OPEN_MENU":
            self._press("start", hold_frames=5)
            return "menu_opened"
